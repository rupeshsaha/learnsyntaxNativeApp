...
const QuizContent = ({ route }) => {
  ...
  if (!content || !quizData) return null;

  const scorePercentage = quizData.best_score ? (quizData.best_score / 100) * 100 : 0;
  ...
  return (
    <SafeAreaView style={{ backgroundColor: "#0a0a0a", flex: 1 }}>
      ...
      >
        ...
        {/* Start Button */}
        <Pressable
          onPress={()=>navigation.navigate("quiz_questions",{id:content.id})}
          style={({ pressed }) => ({
            backgroundColor: "#8b5cf6",
            borderRadius: 16,
            paddingVertical: 18,
